...
的终止信号时，会先优雅停止全部托管服务。每次启动还会建立独立 guardian 租约：POSIX 通过父进程
...
`~/.service-console/controller.json`。文件权限为 `0600`，且只会在托管进程完成清理后删除；若关闭流程
卡住则保留为旧描述文件，原控制器退出后会被 PID 校验拒绝。guardian 租约只保存进程身份与清理元数据，
不会保存命令、环境变量、日志或凭据；POSIX 下文件权限为 `0600`，Windows 使用数据目录的用户 ACL。
进程锁用于避免多个桌面实例互相覆盖。
